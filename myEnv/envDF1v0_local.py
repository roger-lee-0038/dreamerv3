...
class DF1Env0Local(gym.Env):
    ...
    def __init__(self, init_num=1, max_iter=100, index=0):
        ...
        self.action_space = gym.spaces.Box(
            low = 0.0,
            high = 0.1,
            shape = (self.init_num, self.problem.n_var),
            dtype = np.float32,
        )
    ...
    def _get_action(self, action, initial=False):
        if initial:
            ...
            return np.clip(
                        self.current_action + 2*action - 0.1,
                        0,
                        1,
                    )
    
    def _get_obs(self, action):
        F = self.problem.evaluate(action)
        obs = np.hstack([F, self.weights[np.newaxis,:].repeat(self.init_num, axis=0)])
        return np.array(obs, dtype=np.float32)

    def _get_reward(self, obs):
        nor_weights = self.weights * np.array([1, 1])
        val = np.average(np.tensordot(obs[:, :self.n_objs], nor_weights, axes=([-1], [-1])))
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.current_action = self.action_space.sample() * 10
        self.current_obs = self._get_obs(self.current_action)

        nor_weights = self.weights * np.array([1, 1])
        val = np.average(np.tensordot(self.current_obs[:, :self.n_objs], self.weights, axes=([-1], [-1])))
        self.current_val = val
