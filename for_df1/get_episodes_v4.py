...
num_point_episodes = 1000
len_episodes = 20
episodes = []

for i in tqdm(range(num_random_episodes)):
    current_episode = []
    ...
    #print("diff", diff)
    goal_point = goal_point * 0.5
    d_goal = np.sqrt(np.sum(goal_point ** 2))
    lambdas = goal_point / d_goal
    #print("d_goal", d_goal)
    #print("lambdas", lambdas)
    #print()
    start_point = np.hstack([begin_F, begin_point, goal_point])
    current_obs = start_point
    for j in range(len_episodes):
        next_index = np.random.randint(len(allF))
        next_F = allF[next_index]

        d0 = np.dot(next_F - zero_point, lambdas)
        d1 = 0 if d0 < d_goal else d0 - d_goal
        temp_point = zero_point + d0 * lambdas
        d2 = np.sqrt(np.sum((next_F - temp_point) ** 2))
        diff = d1 + d2
        reward = 1 / (1 + diff ** 2)

        next_x = allX[next_index]

        next_obs = np.hstack([next_F, next_x, goal_point])
...
for i in tqdm(range(num_point_episodes)):
    ##print()
    ##print(i)
    current_episode = []
    weights = np.random.rand(2)
